...
import com.dw.ngms.cis.persistence.rowmapper.reservation.ReservationProductionProjectionRowMapper;
...
@Component
@Slf4j
public class ReservationProductionReport implements GenericReport {
    ...
    @Autowired
    public ReservationProductionReport(NamedParameterJdbcTemplate jdbcTemplate,
    ...
    public Resource generateReport(ReportingVm reportingVm) {
        if ("PDF".equalsIgnoreCase(reportingVm.getReportFormat())) {
            ...
            return reportUtils.generatePdfOfReport(htmlReport, "ReservationProductionReport");
        } else if ("CSV".equalsIgnoreCase(reportingVm.getReportFormat())) {
            final List<ReservationProductionProjection> reservationProductionProjections = generateReportData(reportingVm);
    ...
    private String generateReservationProductionHtmlReport(final ReportingVm reportingVm) {
        ...
        final Collection<ReservationProductionProjection> reservationProductionProjections = generateReportData(reportingVm);
        ...
        return templateEngine.process("/reservation/reservation-production-report.html", context);
    }
    private List<ReservationProductionProjection> generateReportData(final ReportingVm reportingVm) {
        ...
        return jdbcTemplate
                .query(sql, parameters, new ReservationProductionProjectionRowMapper());
    }

    private Resource generateCsvResource(List<ReservationProductionProjection> reservationProductionProjection) {
        try {

            final File reservationProductionReport = File.createTempFile("ProductionReport", ".csv", null);
            final Writer writer = new FileWriter(reservationProductionReport);

            StatefulBeanToCsv<ReservationProductionProjection> beanToCsv =
                    new StatefulBeanToCsvBuilder<ReservationProductionProjection>(writer)
                            .withQuotechar(CSVWriter.NO_QUOTE_CHARACTER)
                            .withSeparator(',')
                            .build();

            beanToCsv.write(reservationProductionProjection);
            writer.close();

            return new FileSystemResource(reservationProductionReport);
