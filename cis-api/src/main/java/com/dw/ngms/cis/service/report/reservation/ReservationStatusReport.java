...
import com.dw.ngms.cis.persistence.rowmapper.reservation.ReservationStatusProjectionRowMapper;
...
@Component
@Slf4j
public class ReservationStatusReport implements GenericReport {
    ...
    @Autowired
    public ReservationStatusReport(NamedParameterJdbcTemplate jdbcTemplate,
    ...
    public Resource generateReport(ReportingVm reportingVm) {
        if ("PDF".equalsIgnoreCase(reportingVm.getReportFormat())) {
            ...
            return reportUtils.generatePdfOfReport(htmlReport, "ReservationStatusReport");
        } else if ("CSV".equalsIgnoreCase(reportingVm.getReportFormat())) {
            final List<ReservationStatusProjection> reservationStatusProjections = generateReportData(reportingVm);
            return generateCsvResource(reservationStatusProjections);
        }
    ...
    private String generateReservationProductionHtmlReport(final ReportingVm reportingVm) {
        ...
        final Collection<ReservationStatusProjection> reservationStatusProjections = generateReportData(reportingVm);
        ...
        context.setVariable(DATA, reservationStatusProjections);
        context.setVariable(REQUEST, reportingVm);
        context.setVariable(COLUMNS, columnName);
        return templateEngine.process("/reservation/reservation-status-report.html", context);
    }
    private List<ReservationStatusProjection> generateReportData(final ReportingVm reportingVm) {
        ...
        SqlParameterSource parameters = new MapSqlParameterSource(map);
                ...
                "res.Manager_Name as managerName,\n" +
                "res.Manager_Recived as managerReceived,\n" +
                "res.Manager_TimeTaken as managerTimeTaken,\n" +
                ...
                "res.Resrvation_Status as reservationStatus\n  " +
        ...
        return jdbcTemplate
                .query(sql, parameters, new ReservationStatusProjectionRowMapper());
    }

    private Resource generateCsvResource(List<ReservationStatusProjection> reservationStatusProjection) {
        try {

            final File reservationStatusReport = File.createTempFile("ReservationStatusReport", ".csv", null);
            final Writer writer = new FileWriter(reservationStatusReport);

            StatefulBeanToCsv<ReservationStatusProjection> beanToCsv =
                    new StatefulBeanToCsvBuilder<ReservationStatusProjection>(writer)
                            .withQuotechar(CSVWriter.NO_QUOTE_CHARACTER)
                            .withSeparator(',')
                            .build();

            beanToCsv.write(reservationStatusProjection);
            writer.close();

            return new FileSystemResource(reservationStatusReport);
