...
import com.dw.ngms.cis.persistence.rowmapper.BusinessRuleProjectionRowMapper;
...
@Component
@Slf4j
public class BusinessReport implements GenericReport {
    ...
    @Autowired
    public BusinessReport(NamedParameterJdbcTemplate jdbcTemplate,
    ...
    public Resource generateReport(ReportingVm reportingVm) {
        if ("PDF".equalsIgnoreCase(reportingVm.getReportFormat())) {
            ...
            return reportUtils.generatePdfOfReport(htmlReport, "BusinessRule");
    ...
    private String generateBusinessRuleHtmlReport(final ReportingVm reportingVm) {
        ...
        return templateEngine.process("business-rule.html", context);
    }
    ...
    private List<BusinessRuleProjection> generateReportData(final ReportingVm reportingVm) {
        ...
        String sql = "SELECT PROVINCENAME as province," +
                "PROCESSNAME as processName, " +
                "Caption as moduleName," +
                "DATED as Dated," +
                "USERNAME as userName, " +
                "CONTEXT as context," +
                "TURNAROUNDTIME as turnaroundTime," +
                "NOTIFICATIONTIME as notificationTime," +
                "PREFIX as referenceNumberSample " +
                "from V_WORKFLOW " +
    ...
    private Resource generateCsvResource(List<BusinessRuleProjection> businessRuleProjection) {
        try {
            ...
            final File businessRuleReport = File.createTempFile("BusinessRuleReport", ".csv", null);
